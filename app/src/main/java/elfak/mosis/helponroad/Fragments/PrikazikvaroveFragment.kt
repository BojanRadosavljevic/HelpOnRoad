...
import elfak.mosis.helponroad.databinding.FragmentPrikazikvaroveBinding


class PrikazikvaroveFragment : Fragment() , OnMapReadyCallback, GoogleMap.OnInfoWindowClickListener{
    private var _binding: FragmentPrikazikvaroveBinding?=null
    ...
    private  var kvarMap: MutableMap<Marker?,MutableMap<User, Kvar>> = mutableMapOf()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPrikazikvaroveBinding.inflate(inflater, container, false)
        ...
        mapFragment!!.getMapAsync { mMap ->
            ...
            fusedLocationClient.lastLocation.addOnCompleteListener { location ->
                if (location.result != null) {
                    ...
                    if (vm.filtriraniKvaroviUsera != null)
                        setUpMarkers()
                }
    ...
    {
        map?.clear()
        for(ru in vm.filtriraniKvaroviUsera) {
            if (ru.value.latitude != 0.0 && ru.value.latitude != 0.0) {

                val marker = map?.addMarker(
                    MarkerOptions().position(LatLng(ru.value.latitude, ru.value.longitude))
                        .title(ru.key.userName).icon(
                            BitmapDescriptorFactory.fromResource(R.drawable.icons8_marker_64)
                        )
                )
                kvarMap[marker] = vm.filtriraniKvaroviUsera
                marker?.tag = ru.key
            }
        }
    }
    ...
    {
        vm.izabranKvarUser = marker.tag as User
        vm.izabranKvarKvar = vm.filtriraniKvaroviUsera.getValue(marker.tag as User)
        val dialogFragment=KvartagFragment()
        dialogFragment.show(parentFragmentManager,"KvartagFragment")
